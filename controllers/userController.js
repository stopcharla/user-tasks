    ...
    if(utils.validateObject(req.body, ['emailId', 'password'])){
        try{
            ...
            if(Object.keys(userInfo.dataValues).length > 0){
                ...
                if(comparePwd){
                    const tokenId = await authService.createAuthToken({emailId: userInfo.dataValues.emailId})
...
const addUser = async (req, res) => {
    ...
    if(utils.validateObject(req.body, ['emailId', 'name', 'password'])){
        try{
            ...
            const userInfo = {name:req.body.name, emailId: req.body.emailId, password: pwdHash}
